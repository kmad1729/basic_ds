...
def calc_jump_first_order_iterative(n):
    util_stack = [n]

    order_counter = 0
    while len(util_stack) != 0:
        curr_node = util_stack.pop()
        if curr_node == None or curr_node['order'] != -1:
            continue
        curr_node['order'] = order_counter
        order_counter += 1
        util_stack.append(curr_node['next'])
        util_stack.append(curr_node['jump'])
...
class Test_jump_order_iterative(unittest.TestCase):
    ...
    def test_1(self):
        ...
        calc_jump_first_order_iterative(head)
...
class Test_jump_order_recur(unittest.TestCase):
